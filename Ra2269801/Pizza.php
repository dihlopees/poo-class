...
class Pizza extends ItemCardapio
{
    ...
    const descontoPizzaPequena = 10;
    const descontoPizzaMedia = 15;
    const descontoPizzaGrande = 20;
    ...
    public function calcularPrecoFinal(ItemCardapio $item)
    {
        ...
        if ($item->getTamanho() == 'pequena') {
            $precoFinal = $precoBase - self::descontoPizzaPequena;
            $item->setPrecoCalculado($precoFinal);
        } else if ($item->getTamanho() == 'media') {
            $precoFinal = $precoBase - self::descontoPizzaMedia;
            $item->setPrecoCalculado($precoFinal);
        } else {
            $precoFinal = $precoBase - self::descontoPizzaGrande;
            $item->setPrecoCalculado($precoFinal);
        }
